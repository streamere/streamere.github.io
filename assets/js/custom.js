...
$(function() {
	...
	$("#amazon").hide();
	$("#lyric").hide();
	$("#downloadmp3,.btn-download-mp3").hide();
	$("#shareMenu").hide();
	$("#nowPlaying").hide();
	...
	/* BUTTONS TEMPLATES */
	$(".btn-clear-playlist").click(function(event) {
		clearPlaylist();
	});
	$(".btn-add-music-folder").click(function(event) {
		$('#savePlaylistModal').modal('show');
	});
	/* END BUTTONS TEMPLATES */
	...
	$("#menuLoginRegister, .btn-register").click(function(event) {
		$("#loginModal").modal("show");
		$(".btnregister").click();
	});	
...
{
	try{
		var video = ytplayer.getVideoUrl();
		console.log("URL Video => "+video);
		console.log(ytplayer);
		video 		= video.replace("feature=player_embedded&","");
		var videoID = youtube_parser(video);
		var download_service2 = download_service;
		download_service2 = download_service2.replace("%youtube_url%",video);
		download_service2 = download_service2.replace("%youtube_video%",videoID);
		
		window.open(download_service2);
		//window.open("http://youtube.andthemusic.net?url="+encodeURIComponent(video.replace("feature=player_embedded&","")));
	}catch(e)
	{
		console.log("error download mp3");
		console.log(e);
	}
}
...
function playNextSong(index)
{
	...
	$("#lyric").show();
	$("#downloadmp3,.btn-download-mp3").show();
	$("#shareMenu").show();
	//$("#nowPlaying").show();
