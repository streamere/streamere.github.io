...
class Admin extends CI_Model 
{
    ...
    {
        return $this->db->query("SELECT COUNT(artist) AS TotalRows,artist,picture FROM activity  WHERE action ='1' AND picture != '' AND date > NOW() - INTERVAL 30 DAY group by artist,picture order by TotalRows DESC LIMIT ".INTVAL($this->config->item("items_top")));
    }
      function getTopTrackActivity()
    {
        return $this->db->query("SELECT COUNT(*) AS TotalRows,artist,track,picture FROM activity  WHERE action ='1' AND picture != '' AND date > NOW() - INTERVAL 30 DAY  group by artist,track,picture order by TotalRows DESC LIMIT ".INTVAL($this->config->item("items_top")));
    }
